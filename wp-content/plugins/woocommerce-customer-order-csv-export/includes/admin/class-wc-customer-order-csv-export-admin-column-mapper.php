 ...
 * @since 4.0.0
 */
class WC_Customer_Order_CSV_Export_Admin_Column_Mapper {


	/**
	 * Setup admin column mapper class
	 *
	 * @since 4.0.0
	 */
	public function __construct() {

		// Render a custom column mapper form control when using woocommerce_admin_fields()
		add_action( 'woocommerce_admin_field_wc_customer_order_csv_export_column_mapping', array( $this, 'render_column_mapping' ) );
	}
	...
	public function get_sections() {
		...
		/**
		 * Allow actors to change the sections for column mapper
		 *
		 * @since 4.0.0
		 * @param array $sections
		 */
		return apply_filters( 'wc_customer_order_csv_export_column_mapper_sections', $sections );
	}
	...
	public static function get_settings( $section_id = null ) {
		...
		$settings = array(
			...
			'orders' => array(
				...
				array(
					...
					'type'        => 'wc_customer_order_csv_export_column_mapping',
					'export_type' => 'orders',
					'default'     => self::get_default_column_mapping( 'orders' ),
				),
			...
			'customers' => array(
				...
				array(
					...
					'default'     => self::get_default_column_mapping( 'customers' ),
				),
		...
		/**
		 * Allow actors to add or remove settings from the CSV export column mapping settings page.
		 *
		 * @since 4.0.0
		 * @param array $settings an array of settings for the given section
		 * @param string $section_id current section ID
		 */
		return apply_filters( 'wc_customer_order_csv_export_column_mapping_settings', $found_settings, $section_id );
	}


	/**
	 * Get default column mapping for the given export type
	 *
	 * @since 4.0.0
	 * @param string $export_type Export type
	 * @return array
	 */
	private static function get_default_column_mapping( $export_type ) {
	...
	public function render_column_mapping( $options ) {
		...
		?>
		<tr valign="top">
			<td class="forminp wc-customer-order-csv-export-column-mapping-container" colspan="2">

				<input type="hidden" name="<?php echo esc_attr( $options['id'] ); ?>" value="" />

				<table class="wc-customer-order-csv-export-column-mapping widefat" cellspacing="0">
					<thead>
						<tr>
							<?php
								/**
								 ...
								 */
								$fields = apply_filters( 'wc_customer_order_csv_export_column_mapping_fields', array(
									'sort'         => '',
					...
					<tbody>
						...
						foreach ( $mapping as $mapping_key => $column ) {
							...
							echo '<tr class="column-mapping column-mapping-' . esc_attr( $mapping_key ) . '">';
								...
								switch ( $field ) {
									...
									case 'check-column' :
										echo '<td width="1%" class="check-column">
											<input type="checkbox" class="js-select-column" />
										</td>';
									break;

									case 'name' :
										echo '<td class="name">
											<input type="text" name="' . esc_attr( $options['id'] ) . '[' . esc_attr( $mapping_key ) . '][' . esc_attr( $field ) . ']" value="' . esc_attr( $column[ $field ] ) . '" class="js-column-name" />
										</td>';
									break;
									...
									case 'source' :
										...
										<td class="data">
											...
											<select name="<?php echo esc_attr( $options['id'] ); ?>[<?php echo esc_attr( $mapping_key ); ?>][source]" class="js-column-key wc-enhanced-select-nostd <?php echo $enhanced; ?>">
											...
											<label class="js-column-meta-key-label <?php echo ( 'meta' !== $value ? 'hide' : '' ); ?>">
												<?php esc_html_e( 'Meta key:', 'woocommerce-customer-order-csv-export' ); ?>
												<input type="text" name="<?php echo esc_attr( $options['id'] ); ?>[<?php echo esc_attr( $mapping_key ); ?>][meta_key]" value="<?php echo esc_attr( $meta_key ); ?>" class="js-column-meta-key" />
											</label>
						...
						?>
						<tr class="no-column-mappings <?php if ( count( $mapping ) > 1 ) { echo 'hide'; } ?>">
					...
					<tfoot>
						<tr>
							<td colspan="<?php echo count( $fields ); ?>">
								<a class="button js-add-column-mapping" href="#"><?php esc_html_e( 'Add column', 'woocommerce-customer-order-csv-export' ); ?></a>
								<a class="button js-remove-column-mapping <?php if ( count( $mapping ) < 2 ) { echo 'hide'; } ?>" href="#"><?php esc_html_e( 'Remove selected column(s)', 'woocommerce-customer-order-csv-export' ); ?></a>
	...
	/**
	 * Output sections for column mapper
	 *
	 * @since 4.0.0
	 */
	public function output_sections() {
		...
		foreach ( $sections as $id => $label ) {
			echo '<li><a href="' . admin_url( 'admin.php?page=wc_customer_order_csv_export&tab=columns&section=' . sanitize_title( $id ) ) . '" class="' . ( $current_section === $id ? 'current' : '' ) . '">' . esc_html( $label ) . '</a> ' . ( end( $section_ids ) === $id ? '' : '|' ) . ' </li>';
		}
